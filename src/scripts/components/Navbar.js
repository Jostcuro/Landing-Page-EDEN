...
export async function initNavbar() {
    ...
    <nav class="relative h-16 md:h-20 glass transition-all duration-300" role="navigation" aria-label="Navegación principal">
      <div class="container h-full flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          ...
          <button class="md:hidden p-2 text-eden-700" aria-label="Menú" id="mobile-menu-btn">
      ...
      <div class="md:hidden hidden overflow-hidden transition-all duration-300 ease-in-out" id="mobile-menu" role="dialog" aria-label="Menú móvil">
        <div class="py-6 px-4 space-y-4 border-t border-eden-200">
          ${NAV_LINKS.map(link => `
            <a href="${link.href}" class="block px-4 py-3 text-base font-medium text-eden-700 hover:text-eden-900 hover:bg-eden-100 rounded-lg transition-colors" data-gsap="fade-in">${link.label}</a>
          `).join('')}
  ...
  window.addEventListener('scroll', () => {
    ...
    if (currentScroll > 100) {
      header.querySelector('nav').classList.add('bg-white/95', 'shadow-sm')
    } else {
      header.querySelector('nav').classList.remove('bg-white/95', 'shadow-sm')
    }
